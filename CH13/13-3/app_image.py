...
def message_text(event):
    ...
    google_gemini_result = google_gemini(event.message.text)
        ...
        line_bot_api.reply_message_with_http_info(
            ReplyMessageRequest(
                ...
                messages=[
                    ...
                    TextMessage(text=google_gemini_result),
                ],
            )
        )
...
def google_gemini(user_input):
    upload_image = PIL.Image.open(UPLOAD_FOLDER + "/" + IMAGE_NAME)
    try:
        response = model.generate_content(
            [user_input + "。請用繁體中文回答", upload_image]
        )
